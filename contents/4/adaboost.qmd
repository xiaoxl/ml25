...
This dataset is from [UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/47/horse+colic). The data is about whether horses survive if they get a disease called Colic. The dataset is preprocessed, and the result can be downloaded [here](assests/datasets/horse_colic_clean.csv). Note that based on the description, we only use the first 23 columns, and we change the original 3 outcomes (lived, died was euthanized) into 2 outcomes (lived, died) by combining outcome 2 and 3.
